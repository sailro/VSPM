...
namespace VisualStudio.Package.Manager
{
	public static class VisualStudioRegistry
	{
		...
		private static IEnumerable<T> ReadRegistryEntities<T>(RegistryKey rootKey, string subKeyName, Func<string, bool> predicate,
			Func<RegistryKey, string, T> reader) where T : new()
		{
